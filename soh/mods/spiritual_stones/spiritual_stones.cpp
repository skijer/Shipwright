...
void SpawnStatuesForCurrentScene() {
    ...
    for (int i = 0; i < SPIRITUAL_STONE_COUNT; ++i) {
        ...
        if (w.entranceId < 0)
            continue;
        if (w.sceneId != gPlayState->sceneNum)
            continue;
...
void BuildWarpMessage(int stone, uint16_t* textId, bool* loadFromMessageTable) {
    ...
    std::string body = std::string("Warp to your %g") + StoneNameEnglish(stone) + "%w waypoint?\x1B%gOK&No%w";
...
void OnOpenTextDispatch(uint16_t* textId, bool* loadFromMessageTable) {
    if (!StonesEnabled())
        return;
    if (*textId < kStoneWarpTextIdBase)
        return;
...
extern "C" void SpiritualStone_TickHold(PlayState* play, Player* player) {
    ...
    for (int stone = 0; stone < SPIRITUAL_STONE_COUNT; ++stone) {
        ...
        if (held) {
            ...
            sHoldFrames[stone] = 0;
            if (frames > 0 && frames < SPIRITUAL_STONE_SUMMON_HOLD_FRAMES && gState.warp[stone].entranceId >= 0 &&
                sPendingWarpStone < 0) {
