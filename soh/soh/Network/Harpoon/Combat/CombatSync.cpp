...
void LoadDamageTable(const nlohmann::json& damageTableJson) {
    ...
    SPDLOG_INFO("[HarpoonCombat] damage table loaded ({} entries)", (int)sDamageById.size());
}
...
void TickLocal() {
    ...
    if (c.combatBurnDotFrames > 0) {
        ...
        if (lp != nullptr && (c.combatBurnDotFrames % 4) == 0) {
            ...
            EffectSsEnFire_SpawnVec3f(gPlayState, &lp->actor, &firePos, 60, 0, 0, -1);
        }
        ...
        if (lp != nullptr) {
            if (lp->linearVelocity < 10.0f)
                lp->linearVelocity = 10.0f;
        }
    ...
    if (lp != nullptr) {
        ...
        if (shielding) {
            if (c.combatShieldRaiseFrames < 255)
                c.combatShieldRaiseFrames++;
        } else {
...
void BroadcastApplyStatus(uint32_t targetCid, HarpoonStatusEffect effect, int16_t amount, uint16_t durationFrames,
                          uint32_t sourceCid) {
    if (!HasInstance())
        return;
    nlohmann::json d;
...
void BroadcastShieldParry(uint32_t parryingCid, uint32_t attackerCid, HarpoonShieldKind shieldType,
                          HarpoonParryEffect effect, HarpoonWeaponId weaponSource) {
    if (!HasInstance())
        return;
    nlohmann::json d;
...
void BroadcastAuraTick(uint32_t ownerCid, HarpoonUtilityHitKind kind, float x, float y, float z) {
    if (!HasInstance())
        return;
    nlohmann::json d;
    d["ownerCid"] = ownerCid;
...
void BroadcastUtilityHit(uint32_t attackerCid, uint32_t targetCid, HarpoonUtilityHitKind kind, int32_t ix, int32_t iy,
                         int32_t iz, float fx, float fy, float fz) {
    if (!HasInstance())
        return;
    nlohmann::json d;
    d["attackerCid"] = attackerCid;
...
void HandleApplyStatus(const nlohmann::json& data) {
    ...
    switch (eff) {
        case STATUS_BURN_DOT: {
            // Goron form = immune.
            if (c.transformation == 1 /* MM_PLAYER_FORM_GORON */)
                break;
            ...
            for (int i = 0; i < 6; i++) {
                ...
                EffectSsEnFire_SpawnVec3f(gPlayState, &lp->actor, &firePos, 80, 0, 0, -1);
            }
            Actor_SetColorFilter(&lp->actor, 0x4000, 0xFF, 0, (s16)duration);
            // Knockback push from the fire hit + small damage on apply.
            func_80837C0C(gPlayState, lp, HARPOON_HIT_RESPONSE_FIRE, 5.0f, 6.0f, 0, 20);
            Audio_PlaySoundGeneral(NA_SE_EV_BURNING, &lp->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
        ...
        case STATUS_FREEZE: {
            ...
            func_80837C0C(gPlayState, lp, PLAYER_HIT_RESPONSE_FROZEN, 0.0f, 0.0f, 0, (s32)duration);
            ...
            for (int i = 0; i < 8; i++) {
                ...
                EffectSsEnIce_Spawn(gPlayState, &icePos, 0.5f, &iceVel, &iceAcc, &primIce, &envIce, 60);
            }
            Audio_PlaySoundGeneral(NA_SE_PL_FREEZE_S, &lp->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
                                   &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
            break;
        }
        case STATUS_BLINDNESS: {
            ...
            Audio_PlaySoundGeneral(NA_SE_EN_FANTOM_LAUGH, &lp->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
                                   &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
            break;
        }
        case STATUS_HEAL: {
            ...
            gSaveContext.health = (s16)std::min((int)gSaveContext.healthCapacity, (int)gSaveContext.health + heal * 16);
            break;
        }
        case STATUS_DRAIN: {
...
void HandleShieldParry(const nlohmann::json& data) {
    ...
    HarpoonWeaponId weaponSource = (HarpoonWeaponId)data.value("weaponSource", (int)HARPOON_WEAPON_UNKNOWN);
    (void)shieldType;
    (void)weaponSource;
    ...
    switch (effect) {
        case PARRY_FREEZE_AOE: {
            ...
            if (pit == Harpoon::Instance->clients.end())
                break;
            Player* lp = GET_PLAYER(gPlayState);
            if (lp == nullptr)
                break;
        ...
        case PARRY_SOUL_DRAIN: {
            ...
            if (parryingCid == OwnCid()) {
                gSaveContext.health = (s16)std::min((int)gSaveContext.healthCapacity, (int)gSaveContext.health + 8);
            }
            break;
        }
        case PARRY_REFLECT:
...
void HandleUtilityHit(const nlohmann::json& data) {
    ...
    switch (kind) {
        ...
        case UTIL_FAIRY_HEAL_TOUCH: {
            if (weAreTarget) {
                gSaveContext.health = (s16)std::min((int)gSaveContext.healthCapacity, (int)gSaveContext.health + 16);
            }
            break;
        }
...
void ApplyLocalHit(HarpoonWeaponId weapon, uint32_t attackerCid) {
    ...
    // Heal-class
    if (dmg < 0) {
        gSaveContext.health = (s16)std::min((int)gSaveContext.healthCapacity, (int)gSaveContext.health + (-dmg) * 16);
        return;
    }
    ...
    switch (weapon) {
        ...
        case W_ITM_FIRE_ROD_PROJ: {
            if (c && c->transformation != 1 /* Goron */) {
                c->combatBurnDotFrames = weapon == W_SW97_MAGIC_FIRE ? d.burnLongFrames : d.burnShortFrames;
            }
            break;
        }
        case W_SW97_MAGIC_ICE:
        ...
        case W_ITM_ZONAI_PERMAFROST: {
            uint16_t fr = (weapon == W_SW97_MAGIC_ICE || weapon == W_ITM_ZONAI_PERMAFROST) ? d.freezeLongFrames
                                                                                           : d.freezeShortFrames;
            if (c)
                c->combatFreezeFrames = fr;
        ...
        case W_SW97_MAGIC_DARK: {
            if (c)
                c->combatBlindnessFrames = d.blindnessLongFrames;
            break;
        }
        case W_SW97_ARROW_DARK: {
            if (c)
                c->combatBlindnessFrames = d.blindnessShortFrames;
            break;
        }
...
void ApplyStatusFromAttacker(uint32_t targetCid, Actor* attacker) {
    if (attacker == nullptr || Harpoon::Instance == nullptr)
        return;
    s16 id = attacker->id;

    // Magic spells — wide-area / heavy
    if (id == gSw97ActorId_MagicFire) {
        BroadcastApplyStatus(targetCid, STATUS_BURN_DOT, 1, 180, Harpoon::Instance->ownClientId);
    } else if (id == gSw97ActorId_MagicIce) {
        BroadcastApplyStatus(targetCid, STATUS_FREEZE, 0, 300, Harpoon::Instance->ownClientId);
    } else if (id == gSw97ActorId_MagicDark) {
        BroadcastApplyStatus(targetCid, STATUS_BLINDNESS, 0, 600, Harpoon::Instance->ownClientId);
        ...
        BroadcastApplyStatus(targetCid, STATUS_HEAL, 3 * 16, 0, Harpoon::Instance->ownClientId);
    } else if (id == gSw97ActorId_MagicSoul) {
        BroadcastApplyStatus(targetCid, STATUS_DRAIN, 16, 0, Harpoon::Instance->ownClientId);
    } else if (id == gSw97ActorId_MagicWind) {
        BroadcastApplyStatus(targetCid, STATUS_PUSH, 0, 30, Harpoon::Instance->ownClientId);
    }
    // Elemental arrows — narrower windows
    else if (id == gSw97ActorId_ArrowFire) {
        BroadcastApplyStatus(targetCid, STATUS_BURN_DOT, 1, 120, Harpoon::Instance->ownClientId);
    } else if (id == gSw97ActorId_ArrowIce) {
        BroadcastApplyStatus(targetCid, STATUS_FREEZE, 0, 180, Harpoon::Instance->ownClientId);
    } else if (id == gSw97ActorId_ArrowDark) {
        BroadcastApplyStatus(targetCid, STATUS_BLINDNESS, 0, 300, Harpoon::Instance->ownClientId);
    } else if (id == gSw97ActorId_ArrowLight) {
        BroadcastApplyStatus(targetCid, STATUS_HEAL, 3 * 16, 0, Harpoon::Instance->ownClientId);
    } else if (id == gSw97ActorId_ArrowSoul) {
        BroadcastApplyStatus(targetCid, STATUS_DRAIN, 16, 0, Harpoon::Instance->ownClientId);
    } else if (id == gSw97ActorId_ArrowWind) {
        BroadcastApplyStatus(targetCid, STATUS_PUSH, 0, 20, Harpoon::Instance->ownClientId);
    }
}
...
void HarpoonCombat_BroadcastShieldParry_C(int shieldType, int effect) {
    ...
    HarpoonCombat::BroadcastShieldParry(
        parryingCid, 0u /* attackerCid unknown at parry time */, (HarpoonCombat::HarpoonShieldKind)shieldType,
        (HarpoonCombat::HarpoonParryEffect)effect, HarpoonCombat::HARPOON_WEAPON_UNKNOWN);
}
...
void HarpoonCombat_BroadcastShieldRevive_C(int restoredHealth) {
    if (Harpoon::Instance == nullptr)
        return;
    HarpoonCombat::BroadcastShieldRevive(Harpoon::Instance->ownClientId, (int16_t)restoredHealth);
}
...
void HarpoonCombat_ApplyLocalHit_C(int weaponId, unsigned int attackerCid) {
    HarpoonCombat::ApplyLocalHit((HarpoonCombat::HarpoonWeaponId)weaponId, attackerCid);
}
...
void HarpoonCombat_BroadcastApplyStatus_C(unsigned int targetCid, int effect, int amount, int durationFrames) {
    if (Harpoon::Instance == nullptr)
        return;
    HarpoonCombat::BroadcastApplyStatus(targetCid, (HarpoonCombat::HarpoonStatusEffect)effect, (int16_t)amount,
                                        (uint16_t)durationFrames, Harpoon::Instance->ownClientId);
}

// Broadcast a utility hit (hookshot pull, switch hook swap, gust blow,
// fairy heal touch, zora barrier shock, lantern reveal).
void HarpoonCombat_BroadcastUtilityHit_C(unsigned int targetCid, int kind, float fx, float fy, float fz) {
    if (Harpoon::Instance == nullptr)
        return;
    HarpoonCombat::BroadcastUtilityHit(Harpoon::Instance->ownClientId, targetCid,
                                       (HarpoonCombat::HarpoonUtilityHitKind)kind, 0, 0, 0, fx, fy, fz);
}
