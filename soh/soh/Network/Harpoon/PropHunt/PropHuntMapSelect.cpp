...
constexpr MapDef kMaps[] = {
    { "Kakariko Village", "Village streets, rooftops, and the graveyard.", 0x0DB,
      "map_select/thumbnail_kakariko_village.png", "map_select/sheikah_icon.png" },
    { "Death Mountain", "Volcanic trail with boulders and narrow paths.", 0x13D,
      "map_select/thumbnail_death_mountain.png", "map_select/goron_icon.png" },
    { "Bottom of the Well", "Dark dungeon beneath Kakariko Village.", 0x0098,
      "map_select/thumbnail_bottom_of_the_well.png", "map_select/sheikah_icon.png" },
    { "Gerudo Fortress", "Desert compound with courtyards and corridors.", 0x129,
      "map_select/thumbnail_gerudo_fortress.png", "map_select/gerudo_icon.png" },
    { "Forest Temple", "Twisted corridors in the Sacred Forest Meadow.", 0x169,
      "map_select/thumbnail_forest_temple.png", "map_select/kokori_icon.png" },
    { "Zora's River", "Winding river through Zora's Domain.", 0x0EA, "map_select/thumbnail_zora_river.png",
      "map_select/zora_icon.png" },
    { "Dodongo's Cavern", "Dark dungeon corridors. Cramped and deadly.", 0x004,
      "map_select/thumbnail_dodongo_cavern.png", "map_select/goron_icon.png" },
    { "Ganon's Castle", "Final dungeon. Lava and shadow.", 0x467, "map_select/thumbnail_ganon_castle.png",
      "map_select/hyrule_icon.png" },
    { "Kokiri Forest", "Village, Lost Woods, and Sacred Forest Meadow.", 0x0EE,
      "map_select/thumbnail_kokiri_forest.png", "map_select/kokori_icon.png" },
    { "RANDOM", "Pick a random map!", -1, nullptr, nullptr },
};
...
class PropHuntMapSelectWindow final : public Ship::GuiWindow {
  public:
    ...
    void Draw() override {
        ...
        if (localSel < 0 || localSel >= kMapCount)
            localSel = 0;
        ...
        for (int i = 0; i < kMapCount; i++) {
            ...
            if (kMaps[i].thumbnailPath) {
                ...
                if (i != localSel)
                    dl->AddRectFilled(tl, br, IM_COL32(0, 0, 0, 60));
                ImGui::SetWindowFontScale(1.2f);
                const char* rnd = "RANDOM";
                ImVec2 rs = ImGui::CalcTextSize(rnd);
                dl->AddText(ImVec2(x + (cellW - rs.x) * 0.5f + 1, tl.y + 5 + 1), IM_COL32(0, 0, 0, 200), rnd);
                dl->AddText(ImVec2(x + (cellW - rs.x) * 0.5f, tl.y + 5), IM_COL32(255, 215, 0, 255), rnd);
                ImGui::SetWindowFontScale(2.8f);
                const char* q = "?";
                ImVec2 qs = ImGui::CalcTextSize(q);
                dl->AddText(ImVec2(x + (cellW - qs.x) * 0.5f + 1, y + (cellH - qs.y) * 0.55f + 1),
                            IM_COL32(0, 0, 0, 150), q);
                dl->AddText(ImVec2(x + (cellW - qs.x) * 0.5f, y + (cellH - qs.y) * 0.55f), IM_COL32(255, 255, 255, 180),
                            q);
                ImGui::SetWindowFontScale(1.0f);
            }
            ...
            if (i == localSel) {
                ...
                dl->AddRect(ImVec2(tl.x - 2, tl.y - 2), ImVec2(br.x + 2, br.y + 2), IM_COL32(255, 50, 50, 120), 0, 0,
                            2.0f);
            }
        }
        ...
        if (localSel < kMapCount && kMaps[localSel].thumbnailPath) {
            ...
            dl->AddText(ImVec2(prevX + (prevW - bqs.x) * 0.5f, prevY + (prevH - bqs.y) * 0.5f),
        ...
        if (signTex) {
            dl->AddImage(signTex, ImVec2(signX, signY), ImVec2(signX + signSize, signY + signSize));
        }
        ...
        if (gPlayState != nullptr && !sHasVoted && canInteract) {
            ...
            if (sStickDebounce == 0 && (abs(sx) > 30 || abs(sy) > 30)) {
                ...
                if (sx > 30)
                    col = (col + 1) % kGridCols;
                if (sx < -30)
                    col = (col + kGridCols - 1) % kGridCols;
                if (sy > 30)
                    row = (row + kGridRows - 1) % kGridRows;
                if (sy < -30)
                    row = (row + 1) % kGridRows;
...
void RegisterMapSelectWindow() {
    ...
    if (gui->GetGuiWindow(kName) != nullptr)
        return;
    sWindow = std::make_shared<PropHuntMapSelectWindow>("gOpenWindows.PropHuntMapSelect", kName);
