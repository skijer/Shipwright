...
bool LoadMap(const std::filesystem::path& mapsDir, const MapInit& init, HarpoonTriforceThief::MapDef& out) {
    out.id = init.id;
    out.name = init.name;
...
const MapDef* GetMap(s32 mapIdx) {
    if (mapIdx < 0 || (size_t)mapIdx >= sMaps.size())
        return nullptr;
    return &sMaps[mapIdx];
}
...
const SpawnPoint* GetSpawnPoint(s32 mapIdx, s32 spawnIdx) {
    ...
    if (!m)
        return nullptr;
    if (spawnIdx < 0 || (size_t)spawnIdx >= m->spawnPoints.size())
        return nullptr;
...
bool IsLocalCarrier() {
    return Harpoon::Instance != nullptr && sLocal.carrierClientId != 0 &&
           sLocal.carrierClientId == Harpoon::Instance->ownClientId;
}
bool IsInMapSelect() {
    return sLocal.inMapSelect;
}
bool IsInRound() {
    return sLocal.inRound;
}
...
void HandleMapSelectBegin(const nlohmann::json& p) {
    ...
    if (Harpoon::Instance != nullptr) {
        ...
        Harpoon::Instance->mapSelectMode =
            (HarpoonMapSelectMode)p.value("mapSelectMode", (int)Harpoon::Instance->mapSelectMode);
...
void HandleMapConfirmed(const nlohmann::json& p) {
    ...
    Notification::Emit({
        ...
        .message = "Round " + std::to_string(sLocal.roundIndex) + " — " + (m ? m->name : std::string("?")),
        .remainingTime = 4.0f,
    });
...
void HandleTriforceDrop(const nlohmann::json& p) {
    ...
    if (dropperTeam == "red") {
        ...
        if (sLocal.redTimerFrames > capFrames)
            sLocal.redTimerFrames = capFrames;
    } else if (dropperTeam == "blue") {
        sLocal.blueTimerFrames = sLocal.carrierTimerFrames + 10 * 20;
        if (sLocal.blueTimerFrames > capFrames)
            sLocal.blueTimerFrames = capFrames;
    }
...
void HandleRoundResult(const nlohmann::json& p) {
    ...
    SPDLOG_INFO("[Harpoon][TriforceThief] round {} ended winner=cid{} team='{}'", round, winner, winnerTeam);
    (void)winner;
    ...
    // leaderboard modal.
    if (!winnerTeam.empty()) {
        std::string label = (winnerTeam == "red") ? "Red" : (winnerTeam == "blue") ? "Blue" : winnerTeam;
        Notification::Emit({
            .prefix = "Triforce Thief",
    ...
    if (gPlayState != nullptr) {
        ...
        if (localPlayer != nullptr) {
            localPlayer->stateFlags1 &=
                ~(PLAYER_STATE1_DEAD | PLAYER_STATE1_IN_ITEM_CS | PLAYER_STATE1_IN_CUTSCENE | PLAYER_STATE1_LOADING);
...
void HandleEvent(const nlohmann::json& envelope) {
    ...
    if (evt == kEvtMapSelectBegin)
        HandleMapSelectBegin(data);
    else if (evt == kEvtMapHover)
        HandleMapHover(data);
    else if (evt == kEvtMapConfirmed)
        HandleMapConfirmed(data);
    else if (evt == kEvtTriforceSpawn)
        HandleTriforceSpawn(data);
    else if (evt == kEvtTriforcePickup)
        HandleTriforcePickup(data);
    else if (evt == kEvtTriforceDrop)
        HandleTriforceDrop(data);
        ...
        if (ws < 5)
            ws = 5;
        if (ws > 99)
            ws = 99; // team-mode cap per user spec
        ...
        if (Harpoon::Instance != nullptr && targetId != 0) {
            ...
            if (it != Harpoon::Instance->clients.end()) {
                ...
                if (it->second.player != nullptr) {
                    ...
                    NameTag_RegisterForActorWithOptions(a, it->second.name.c_str(), opts);
                }
            }
...
bool PickTriforceAnchor(Vec3f* outPos) {
    ...
    static const s16 kAnchorIds[] = { ACTOR_DOOR_ANA, ACTOR_OBJ_BEAN, ACTOR_OBJ_MURE2, ACTOR_EN_A_OBJ };
...
void OnSceneLoaded() {
    ...
    bool isHost = (Harpoon::Instance != nullptr) && (Harpoon::Instance->ownClientId == Harpoon::Instance->hostClientId);
    if (isHost && sLocal.pendingAnchorPick) {
        ...
        Harpoon::Instance->SendJsonToRemote(BuildTriforceSpawnPayload(
            sLocal.pendingAnchorMap, sLocal.currentSpawn, sLocal.triforceX, sLocal.triforceY, sLocal.triforceZ));
        sLocal.cutsceneTimer = 100;
        sLocal.cutsceneReady = false; // flipped just below
        sLocal.cutsceneSubCam = SUBCAM_FREE;
        Harpoon::Instance->SendJsonToRemote(
            BuildCutsceneBeginPayload(sLocal.triforceX, sLocal.triforceY, sLocal.triforceZ));
...
nlohmann::json BuildTriforceSpawnPayload(s32 mapIdx, s32 spawnIdx, f32 x, f32 y, f32 z) {
    nlohmann::json d;
    d["mapIndex"] = mapIdx;
...
nlohmann::json BuildTriforceDropPayload(u32 dropperClientId, f32 startX, f32 startY, f32 startZ, f32 velX, f32 velY,
                                        f32 velZ) {
    nlohmann::json d;
...
nlohmann::json BuildRoundResultPayload(u32 winnerClientId, s32 roundIndex, const std::string& winnerTeam) {
    nlohmann::json d;
...
void BuildLeaderboardFromPayload(const nlohmann::json& p) {
    sLeaderboard.clear();
    if (!p.contains("leaderboard") || !p["leaderboard"].is_array())
        return;
    ...
    std::sort(sLeaderboard.begin(), sLeaderboard.end(), [](const LeaderboardRow& a, const LeaderboardRow& b) {
...
nlohmann::json BuildStateSyncPayload(u32 carrierClientId, s32 redTimerFrames, s32 blueTimerFrames) {
    nlohmann::json d;
    d["carrierClientId"] = carrierClientId;
...
nlohmann::json BuildTeamAssignPayload(u32 targetClientId, const std::string& team) {
    nlohmann::json d;
...
void SetLocalTeam(const std::string& team) {
    ...
    if (Harpoon::Instance != nullptr && Harpoon::Instance->ownClientId != 0) {
        ...
        Harpoon::Instance->SendJsonToRemote(BuildTeamAssignPayload(Harpoon::Instance->ownClientId, team));
    }
}
...
void DrawHud() {
    ...
    if (sLocal.inRound && !sLocal.roundEnded) {
        ...
        fg->AddText(font, font->FontSize * digitScale, ImVec2(baseX + 1, redY + 1), IM_COL32(0, 0, 0, 200), redStr);
        fg->AddText(font, font->FontSize * digitScale, ImVec2(baseX, redY), IM_COL32(235, 60, 60, 255), redStr);
        ...
        fg->AddText(font, font->FontSize * digitScale, ImVec2(blueX + 1, blueY + 1), IM_COL32(0, 0, 0, 200), blueStr);
        fg->AddText(font, font->FontSize * digitScale, ImVec2(blueX, blueY), IM_COL32(60, 120, 235, 255), blueStr);
    }
    ...
    if (!ImGui::Begin("TriforceThiefHUD", nullptr, flags)) {
        ImGui::End();
        return;
    }
    ...
    if (sLocal.inMapSelect) {
        ...
        for (size_t i = 0; i < sMaps.size(); i++) {
            ...
            ImVec4 c = isHovered ? ImVec4(1.0f, 1.0f, 0.4f, 1.0f) : ImVec4(0.7f, 0.7f, 0.7f, 1.0f);
            ImGui::TextColored(c, "  %s %s", isHovered ? ">" : " ", sMaps[i].name.c_str());
        }
        ImGui::Separator();
        ImGui::TextColored(ImVec4(0.6f, 0.6f, 0.6f, 1.0f), "D-Left / D-Right to cycle");
        ...
        if (sLocal.carrierClientId != 0) {
            ImGui::TextColored(ImVec4(1.0f, 0.85f, 0.2f, 1.0f), "Carrier: cid%u", sLocal.carrierClientId);
        } else {
            ImGui::TextColored(ImVec4(0.5f, 1.0f, 0.5f, 1.0f), "Triforce: dropped");
        }
        ...
        // hold it.)
        if (gPlayState != nullptr && Harpoon::Instance != nullptr && !IsLocalCarrier()) {
            Vec3f world = { sLocal.triforceX, sLocal.triforceY + 30.0f, sLocal.triforceZ };
            ...
            bool onScreen = !behind && vpx >= vp->Pos.x && vpx <= vp->Pos.x + vp->Size.x && vpy >= vp->Pos.y &&
                            vpy <= vp->Pos.y + vp->Size.y;
            ...
            auto drawTriforceIcon = [&](f32 px, f32 py, f32 scale) {
                ...
                f32 s = 12.0f * scale;
                // Top triangle
                fg->AddTriangleFilled(ImVec2(px, py - s), ImVec2(px - s * 0.5f, py), ImVec2(px + s * 0.5f, py), gold);
                // Bottom-left
                fg->AddTriangleFilled(ImVec2(px - s * 0.5f, py), ImVec2(px - s, py + s), ImVec2(px, py + s), gold);
                // Bottom-right
                fg->AddTriangleFilled(ImVec2(px + s * 0.5f, py), ImVec2(px, py + s), ImVec2(px + s, py + s), gold);
            };
            ...
            if (onScreen) {
                ...
                fg->AddTriangleFilled(ImVec2(ax, ay), ImVec2(ax + backX + perpX, ay + backY + perpY),
                                      ImVec2(ax + backX - perpX, ay + backY - perpY), IM_COL32(255, 235, 100, 220));
            }
        }
    } else {
        ImGui::TextColored(ImVec4(0.6f, 0.6f, 0.6f, 1.0f), "Lobby — waiting for host to start.");
    }
    ...
    if (sLocal.preRoundCountdownFrames > 0) {
        ...
        const char* label;
        if (sLocal.preRoundCountdownFrames > 40)
            label = "3";
        else if (sLocal.preRoundCountdownFrames > 20)
            label = "2";
        else
            label = "1";
        ImFont* font = ImGui::GetFont();
        ...
        fg->AddText(font, font->FontSize * scale, ImVec2(px + 3, py + 3), IM_COL32(0, 0, 0, 200), label);
        fg->AddText(font, font->FontSize * scale, ImVec2(px, py), IM_COL32(255, 215, 0, 255), label);
        ...
        fg->AddText(font, font->FontSize * scale, ImVec2(px + 3, py + 3), IM_COL32(0, 0, 0, alpha * 200 / 255), label);
        fg->AddText(font, font->FontSize * scale, ImVec2(px, py), IM_COL32(120, 255, 120, alpha), label);
    }
    ...
    if (sLocal.roundEnded && !sLeaderboard.empty()) {
        ...
        ImGui::SetNextWindowPos(ImVec2(vp->Pos.x + vp->Size.x * 0.5f, vp->Pos.y + vp->Size.y * 0.5f), ImGuiCond_Always,
                                ImVec2(0.5f, 0.5f));
        ImGuiWindowFlags lbFlags = ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove |
                                   ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoCollapse |
                                   ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoInputs;
        ImGui::SetNextWindowBgAlpha(0.85f);
        if (ImGui::Begin("Final Standings", nullptr, lbFlags)) {
            ImGui::TextColored(ImVec4(1.0f, 0.85f, 0.2f, 1.0f), "  ROUND OVER  ");
            ImGui::Separator();
            ...
            for (const auto& r : sLeaderboard) {
                ...
                ImVec4 col = (r.team == "red")    ? ImVec4(0.92f, 0.30f, 0.30f, 1.0f)
                             : (r.team == "blue") ? ImVec4(0.30f, 0.55f, 0.92f, 1.0f)
                                                  : ImVec4(0.7f, 0.7f, 0.7f, 1.0f);
                ImGui::TextColored(col, "    %-16s  %02d:%02d", r.name.c_str(), r.carrySecs / 60, r.carrySecs % 60);
            }
            ImGui::Separator();
            ImGui::TextColored(ImVec4(0.6f, 0.6f, 0.6f, 1.0f), "  Host starts the next round to dismiss.  ");
        }
        ImGui::End();
    }
}
...
void DrawTriforceOnGround(PlayState* play) {
    if (!sLocal.inRound)
        return;
    if (sLocal.carrierClientId != 0)
        return; // Someone is carrying it.
    if (play == nullptr)
        return;

    Gfx* triforceDL = GetTriforceDL();
    if (triforceDL == nullptr)
        return;
    ...
    Matrix_Push();
    Matrix_Translate(sLocal.triforceX, sLocal.triforceY + 30.0f + bobY, sLocal.triforceZ, MTXMODE_NEW);
...
void DrawTriforceAboveHead(Actor* actor, PlayState* play) {
    ...
    Matrix_Push();
    Matrix_Translate(actor->world.pos.x, actor->world.pos.y + 85.0f, actor->world.pos.z, MTXMODE_NEW);
    Matrix_RotateY(sTriforceSpinAngle * (M_PI / 0x8000), MTXMODE_APPLY);
...
bool ShouldPickupTriforce(f32 px, f32 py, f32 pz) {
    ...
    if (sLocal.carrierClientId != 0)
        return false;
    if (sLocal.dropFlyTimer > 0)
        return false; // mid-flight, untouchable
    ...
    //    notification listing the offenders so they know who to remind.
    {
        ...
        for (auto& [cid, c] : Harpoon::Instance->clients) {
            if (!c.online)
                continue;
            if (c.team.empty()) {
                teamless.push_back(c.name.empty() ? ("cid" + std::to_string(cid)) : c.name);
            }
        }
        if (!teamless.empty()) {
            std::string list;
            ...
            SPDLOG_INFO("[Harpoon][TriforceThief] round-start blocked: {} teamless", (s32)teamless.size());
            return;
        }
    }
...
bool StepDropPhysics() {
    ...
    Vec3f nextPos = { sLocal.triforceX + sLocal.dropVelX, sLocal.triforceY + sLocal.dropVelY,
    ...
    s32 wallBgId = 0;
    if (BgCheck_EntitySphVsWall3(&gPlayState->colCtx, &resolvedPos, &nextPos, &prevPos, WALL_RADIUS, &wallPoly,
                                 &wallBgId, nullptr, WALL_HEIGHT)) {
        if (wallPoly != nullptr) {
    ...
    s32 ceilBgId = 0;
    if (sLocal.dropVelY > 0.0f && BgCheck_EntityCheckCeiling(&gPlayState->colCtx, &ceilY, &nextPos, CEILING_HEIGHT,
                                                             &ceilPoly, &ceilBgId, nullptr)) {
        nextPos.y = ceilY;
    ...
    CollisionPoly* floorPoly = nullptr;
    s32 floorBgId = 0;
    Vec3f floorQueryPos = { nextPos.x, fmaxf(sLocal.dropPrevY, nextPos.y) + 20.0f, nextPos.z };
    f32 floorY = BgCheck_EntityRaycastFloor4(&gPlayState->colCtx, &floorPoly, &floorBgId, nullptr, &floorQueryPos);
    bool onFloor = false;
    ...
    f32 speedSq =
        sLocal.dropVelX * sLocal.dropVelX + sLocal.dropVelY * sLocal.dropVelY + sLocal.dropVelZ * sLocal.dropVelZ;
...
void RespawnTriforceRandom() {
    ...
    if (isHost) {
        Harpoon::Instance->SendJsonToRemote(BuildTriforceSpawnPayload(sLocal.confirmedMap, idx, sp.x, sp.y, sp.z));
    }
...
void TickPassiveRegen() {
    ...
    if (gSaveContext.health < gSaveContext.healthCapacity) {
        ...
        gSaveContext.health =
            (s16)((newHealth > gSaveContext.healthCapacity) ? gSaveContext.healthCapacity : newHealth);
    }
}
...
void TickCutscene() {
    ...
    Vec3f eye = { sLocal.triforceX + sinf(angle) * camDist, sLocal.triforceY + camHeight,
...
void TickFrame() {
    ...
    AnimateDropFly();
    for (auto it = sLocal.pickupCooldownByCid.begin(); it != sLocal.pickupCooldownByCid.end();) {
        if (--it->second <= 0)
            it = sLocal.pickupCooldownByCid.erase(it);
        else
            ++it;
    }
        ...
        if (lp != nullptr) {
            ...
            if (lp->actor.world.pos.y < lowestY - 30.0f) {
                ...
                if (wasCarrier && Harpoon::Instance != nullptr) {
                    const MapDef* m = GetMap(sLocal.confirmedMap);
                    f32 nx = sLocal.triforceX, ny = sLocal.triforceY, nz = sLocal.triforceZ;
                    s32 nIdx = -1;
                    Vec3f anchor;
                    ...
                    sLocal.dropFlyTimer = 0;
                    Harpoon::Instance->SendJsonToRemote(
                        BuildTriforceSpawnPayload(sLocal.confirmedMap, nIdx, nx, ny, nz));
                }
    ...
    //       boosted speed.
    {
        bool shouldBuff = (sLocal.inRound && !sLocal.roundEnded && IsLocalCarrier());
        if (shouldBuff && !sLocal.appliedBunnyHood) {
            sLocal.savedSpeedModValue = CVarGetFloat(CVAR_CHEAT("SpeedModifier.Value"), 1.0f);
            sLocal.savedSpeedToggleCVar = CVarGetInteger(CVAR_CHEAT("SpeedModifier.SpeedToggle"), 0);
            sLocal.savedWalkSpeedToggle = gWalkSpeedToggle;
            sLocal.savedBunnyHoodCVar = CVarGetInteger(CVAR_ENHANCEMENT("MMBunnyHood"), BUNNY_HOOD_VANILLA);
            ...
            CVarSetFloat(CVAR_CHEAT("SpeedModifier.Value"), sLocal.savedSpeedModValue);
            CVarSetInteger(CVAR_CHEAT("SpeedModifier.SpeedToggle"), sLocal.savedSpeedToggleCVar);
            gWalkSpeedToggle = sLocal.savedWalkSpeedToggle;
            CVarSetInteger(CVAR_ENHANCEMENT("MMBunnyHood"), sLocal.savedBunnyHoodCVar);
            sLocal.appliedBunnyHood = false;
        }
    }
    ...
    bool isHostForTimer = (Harpoon::Instance != nullptr && Harpoon::Instance->ownClientId != 0 &&
                           Harpoon::Instance->ownClientId == Harpoon::Instance->hostClientId);
    if (isHostForTimer && sLocal.inRound && !sLocal.roundEnded && sLocal.carrierClientId != 0 &&
        sLocal.carrierTimerFrames > 0) {
        s64 nowMs = TT_NowMs();
        ...
        if (sLocal.currentCarrierTeam.empty() && Harpoon::Instance != nullptr) {
            auto it = Harpoon::Instance->clients.find(sLocal.carrierClientId);
            if (it != Harpoon::Instance->clients.end() && !it->second.team.empty()) {
                sLocal.currentCarrierTeam = it->second.team;
            } else {
        ...
        for (s32 t : kThresholds) {
            if (curSec == t && sLocal.lastWarnSecond != t) {
                ...
                Harpoon::Instance->SendJsonToRemote(BuildTimerWarningPayload(t));
                break;
            }
        }
    ...
    //       the room of that team — purely cosmetic for the leaderboard.
    {
        ...
        if (isHostLocal && sLocal.inRound && !sLocal.roundEnded) {
            ...
            if (sLocal.redTimerFrames <= 0)
                winnerTeam = "red";
            else if (sLocal.blueTimerFrames <= 0)
                winnerTeam = "blue";
            if (!winnerTeam.empty()) {
                sLocal.roundEnded = true;
                // Prefer the current carrier as the "winner client id"
                // (used by the leaderboard modal as the highlighted row).
                u32 winnerCid = (sLocal.currentCarrierTeam == winnerTeam) ? sLocal.carrierClientId : 0u;
                if (winnerCid == 0 && Harpoon::Instance != nullptr) {
                    for (auto& [cid, c] : Harpoon::Instance->clients) {
                        if (c.team == winnerTeam) {
                            winnerCid = cid;
                            break;
                        }
                    }
                }
                Harpoon::Instance->SendJsonToRemote(BuildRoundResultPayload(winnerCid, sLocal.roundIndex, winnerTeam));
    ...
    //       which is smooth enough to feel continuous.
    {
        ...
        if (isHostLocal && sLocal.inRound && !sLocal.roundEnded) {
            s64 nowMs = TT_NowMs();
            if (sLocal.lastHostSyncMs == 0 || (nowMs - sLocal.lastHostSyncMs) >= 100) {
                ...
                Harpoon::Instance->SendJsonToRemote(
                    BuildStateSyncPayload(sLocal.carrierClientId, sLocal.redTimerFrames, sLocal.blueTimerFrames));
            }
        }
    }
    ...
    bool isHost =
        (Harpoon::Instance->ownClientId != 0 && Harpoon::Instance->ownClientId == Harpoon::Instance->hostClientId);
    if (isHost && Harpoon::Instance->gameState == HARPOON_STATE_MAP_SELECT &&
        Harpoon::Instance->mapSelectMode == MAP_SELECT_EVERYONE_CHOOSES) {
        if (sLocal.mapVoteDeadline > 0)
            sLocal.mapVoteDeadline--;
    ...
    if (sLocal.inRound && !sLocal.roundEnded && sLocal.carrierClientId == 0 && sLocal.dropFlyTimer == 0 &&
        Harpoon::Instance != nullptr) {
        const MapDef* m = GetMap(sLocal.confirmedMap);
        bool inVoid = false;
        if (m != nullptr && sLocal.currentSpawn >= 0 && (size_t)sLocal.currentSpawn < m->spawnPoints.size()) {
            f32 spawnY = m->spawnPoints[sLocal.currentSpawn].y;
            if (sLocal.triforceY < spawnY - 200.0f)
                inVoid = true;
        }
        ...
        bool onExitPoly = IsPositionOnExitPoly(sLocal.triforceX, sLocal.triforceY, sLocal.triforceZ);
        ...
        s32 threshold = atSpawnPoint ? kRespawnFramesAtSpawn : kRespawnFramesElsewhere;
        ...
        if (inVoid || onExitPoly || sLocal.idleOnGroundFrames >= threshold) {
            ...
            if (isHostNow) {
                ...
                Harpoon::Instance->SendJsonToRemote(
                    BuildTriforceSpawnPayload(sLocal.confirmedMap, newIdx, newX, newY, newZ));
            }
        }
    } else {
        sLocal.idleOnGroundFrames = 0;
...
void BigStartGame() {
    ...
    SPDLOG_INFO("[Harpoon][TriforceThief] BigStartGame entrance=0x{:X}", (u32)gSaveContext.entranceIndex);
}
...
s32 HarpoonTriforceThief_IsLoaded(void) {
    return HarpoonTriforceThief::IsLoaded() ? 1 : 0;
}
s32 HarpoonTriforceThief_IsLocalCarrier(void) {
    return HarpoonTriforceThief::IsLocalCarrier() ? 1 : 0;
}
s32 HarpoonTriforceThief_GetConfirmedMap(void) {
    return HarpoonTriforceThief::GetLocalState().confirmedMap;
}
