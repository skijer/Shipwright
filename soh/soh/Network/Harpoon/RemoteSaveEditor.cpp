...
bool IsHostLocal() {
    if (Harpoon::Instance == nullptr)
        return false;
    return Harpoon::Instance->ownClientId != 0 && Harpoon::Instance->ownClientId == Harpoon::Instance->hostClientId;
}
...
void TplSetUpgValue(HarpoonTemplates::Template* t, int upg, s32 value) {
    t->upgrades = (t->upgrades & gUpgradeNegMasks[upg]) | (((u32)value) << gUpgradeShifts[upg]);
}
...
void DrawUpgradeIconForTemplate(HarpoonTemplates::Template* t, const std::string& categoryName, int categoryId,
    ...
    if (item != ITEM_NONE) {
        ...
        if (ImGui::ImageButton(
                (slotEntry.name + "##remoteUpgBtn").c_str(),
        ...
        if (ImGui::Button("##remoteUpgEmpty", ImVec2(48.0f, 48.0f) + ImGui::GetStyle().FramePadding * 2)) {
            ImGui::OpenPopup(upgradePopupPicker);
        }
    }
    PopStyleButton();
    ...
    if (ImGui::BeginPopup(upgradePopupPicker)) {
        for (size_t pickerIndex = 0; pickerIndex < items.size(); pickerIndex++) {
            ...
            if (items[pickerIndex] == ITEM_NONE) {
                if (ImGui::Button("##remoteUpgNone", ImVec2(48.0f, 48.0f) + ImGui::GetStyle().FramePadding * 2)) {
...
void DrawUpgradeComboForTemplate(HarpoonTemplates::Template* t, const std::string& categoryName, int categoryId,
...
uint16_t* TemplateFlagEntry(HarpoonTemplates::Template* t, FlagTableType type, size_t row) {
    switch (type) {
        case EVENT_CHECK_INF:
            if (row >= ARRAY_COUNT(t->eventChkInf))
                return nullptr;
            return &t->eventChkInf[row];
        case ITEM_GET_INF:
            if (row >= ARRAY_COUNT(t->itemGetInf))
                return nullptr;
            return &t->itemGetInf[row];
        case INF_TABLE:
            if (row >= ARRAY_COUNT(t->infTable))
                return nullptr;
            return &t->infTable[row];
        case EVENT_INF:
            if (row >= ARRAY_COUNT(t->eventInf))
                return nullptr;
            return &t->eventInf[row];
        case RANDOMIZER_INF:
            if (row >= t->randomizerInf.size())
                return nullptr;
...
void DrawInfoTab(HarpoonTemplates::Template* t) {
    ...
    if (t->magic > t->magicCapacity)
        t->magic = t->magicCapacity;
    ...
    if (SliderInt("Time", &dayTimeI, IntSliderOptions().Color(THEME_COLOR).Min(0).Max(0xFFFF).Tooltip("Time of day"))) {
        t->dayTime = (uint16_t)dayTimeI;
    }
    if (Button("Dawn", ButtonOptions().Color(THEME_COLOR).Size(Sizes::Inline)))
        t->dayTime = 0x4000;
    ImGui::SameLine();
    if (Button("Noon", ButtonOptions().Color(THEME_COLOR).Size(Sizes::Inline)))
        t->dayTime = 0x8000;
    ImGui::SameLine();
    if (Button("Sunset", ButtonOptions().Color(THEME_COLOR).Size(Sizes::Inline)))
        t->dayTime = 0xC001;
    ImGui::SameLine();
    if (Button("Midnight", ButtonOptions().Color(THEME_COLOR).Size(Sizes::Inline)))
        t->dayTime = 0;
    ...
    if (Checkbox("Has BGS", &bgs,
                 CheckboxOptions().Color(THEME_COLOR).Tooltip("Is Biggoron sword unlocked? Replaces Giant's knife"))) {
        t->bgsFlag = bgs ? 1 : 0;
    }
    ...
    Combobox("Audio", &t->audioSetting, audioMap, ComboboxOptions().Color(THEME_COLOR).Tooltip("Sound setting"));
    ...
    static const std::array<const char*, 7> minigameHS = { "Horseback Archery", "Big Poe Points",
...
void DrawInventoryTab(HarpoonTemplates::Template* t) {
        ...
        for (int32_t x = 0; x < 6; x++) {
            ...
            if (ImGui::BeginPopup(itemPopupPicker)) {
                ...
                for (size_t pickerIndex = 0; pickerIndex < possibleItems.size(); pickerIndex++) {
                    ...
                    PushStyleButton(Colors::DarkGray);
                    bool ret = ImGui::ImageButton(slotEntry.name.c_str(),
                                                  std::dynamic_pointer_cast<Fast::Fast3dGui>(
                                                      Ship::Context::GetRawInstance()->GetWindow()->GetGui())
                                                      ->GetTextureByName(slotEntry.name),
                                                  ImVec2(kInvImageSize, kInvImageSize), ImVec2(0, 0), ImVec2(1, 1));
    ...
    for (uint32_t ammoIndex = 0, drawnAmmoItems = 0; ammoIndex < 16; ammoIndex++) {
        ...
        if (mapIt != itemMapping.end()) {
            ImGui::Image(
                std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetRawInstance()->GetWindow()->GetGui())
                    ->GetTextureByName(mapIt->second.name),
            ...
            for (int32_t x = 0; x < 6; x++) {
                ...
                if (item != ITEM_NONE) {
                    ...
                    if (it != customItemMapping.end()) {
                        ...
                        if (tex) {
                            clicked =
                                ImGui::ImageButton(slotEntry.name.c_str(), tex, ImVec2(kInvImageSize, kInvImageSize),
                                                   ImVec2(0, 0), ImVec2(1, 1));
                        } else {
                            PushStyleButton(Colors::DarkGray);
                            clicked = ImGui::Button(slotEntry.name.c_str(), ImVec2(kInvImageSize, kInvImageSize) +
                        ...
                        PushStyleButton(Colors::DarkGray);
                        clicked = ImGui::Button(buttonLabel, ImVec2(kInvImageSize, kInvImageSize) +
                                                                 ImGui::GetStyle().FramePadding * 2);
                        PopStyleButton();
                    }
                } else {
                ...
                if (ImGui::BeginPopup(customItemPopupPicker)) {
                    ...
                    for (int32_t pickerIndex = 0; pickerIndex < 24; pickerIndex++) {
                        ...
                        if (it != customItemMapping.end()) {
                            ...
                            if (tex) {
                                ret = ImGui::ImageButton(entry.name.c_str(), tex, ImVec2(kInvImageSize, kInvImageSize),
                                                         ImVec2(0, 0), ImVec2(1, 1));
                            } else {
                                PushStyleButton(Colors::DarkGray);
                                ret = ImGui::Button(entry.name.c_str(), ImVec2(kInvImageSize, kInvImageSize) +
                                                                            ImGui::GetStyle().FramePadding * 2);
                                PopStyleButton();
                            }
                    ...
                    ImGui::Text("Upgrades:");
                    {
                        ...
                        if (it != customItemMapping.end()) {
                            ...
                            PushStyleButton(Colors::DarkGray);
                            ret = ImGui::Button("ITEM_ROCS_CAPE##pickerCapeR", ImVec2(kInvImageSize, kInvImageSize) +
                                                                                   ImGui::GetStyle().FramePadding * 2);
                            PopStyleButton();
                        }
                        if (ret) {
    ...
    if (ImGui::CollapsingHeader("MM Masks Inventory (Page 3)")) {
        ...
        if (ImGui::Button("Give All MM Masks##remote")) {
            for (int i = 0; i < 24; i++)
                t->items[48 + i] = gPage3MaskItems[i];
        }
        ImGui::SameLine();
        ...
        for (int32_t y = 0; y < 4; y++) {
            for (int32_t x = 0; x < 6; x++) {
                ...
                if (tex) {
                    ...
                    bool clicked;
                    if (hasItem) {
                        clicked = ImGui::ImageButton(maskName, tex, ImVec2(kInvImageSize, kInvImageSize), ImVec2(0, 0),
                                                     ImVec2(1, 1));
                    } else {
                        clicked = ImGui::ImageButton(maskName, tex, ImVec2(kInvImageSize, kInvImageSize), ImVec2(0, 0),
                    ...
                    if (clicked) {
                        if (hasItem)
                            t->items[slotIndex] = ITEM_NONE;
                        else
                            t->items[slotIndex] = gPage3MaskItems[visualIndex];
                    }
                } else {
                    char buttonLabel[64];
                    if (hasItem) {
...
void DrawFlagsTab(HarpoonTemplates::Template* t) {
    ...
    if (ImGui::TreeNode("Saved Scene Flags##remote")) {
        ...
        if (ImGui::BeginCombo("##MapR", SohUtils::GetSceneName(selectedSceneFlagMap).c_str())) {
            for (int32_t sceneIndex = 0; sceneIndex < SCENE_ID_MAX && sceneIndex < (int32_t)ARRAY_COUNT(t->sceneFlags);
                 sceneIndex++) {
        ...
        [&]() {
            ...
            if (ImGui::BeginCombo("##GSMapR", gsAreaNames[selectedGsMap])) {
                for (size_t i = 0; i < 22; i++) {
                    if (ImGui::Selectable(gsAreaNames[i]))
                        selectedGsMap = i;
                }
                ImGui::EndCombo();
            }
            ...
            if (!flagFilter.IsActive()) {
                ...
                for (size_t row = 0; row < rows; row++) {
                    ...
                    for (int32_t flagIndex = 15; flagIndex >= 0; flagIndex--) {
                        ...
                        if (!flagFilter.PassFilter(searchable.c_str()))
                            continue;
                        hasMatches = true;
                        ImGui::PushID(index);
...
void DrawEquipmentTab(HarpoonTemplates::Template* t) {
    ...
    const std::vector<uint8_t> bulletBagValues = { ITEM_NONE, ITEM_BULLET_BAG_30, ITEM_BULLET_BAG_40,
                                                   ITEM_BULLET_BAG_50 };
    DrawUpgradeIconForTemplate(t, "Bullet Bag", UPG_BULLET_BAG, bulletBagValues);
    ImGui::SameLine();
    const std::vector<uint8_t> quiverValues = { ITEM_NONE, ITEM_QUIVER_30, ITEM_QUIVER_40, ITEM_QUIVER_50 };
    DrawUpgradeIconForTemplate(t, "Quiver", UPG_QUIVER, quiverValues);
    ImGui::SameLine();
    const std::vector<uint8_t> bombBagValues = { ITEM_NONE, ITEM_BOMB_BAG_20, ITEM_BOMB_BAG_30, ITEM_BOMB_BAG_40 };
    ...
    ImGui::SameLine();
    const std::vector<uint8_t> strengthValues = { ITEM_NONE, ITEM_BRACELET, ITEM_GAUNTLETS_SILVER,
                                                  ITEM_GAUNTLETS_GOLD };
    DrawUpgradeIconForTemplate(t, "Strength", UPG_STRENGTH, strengthValues);

    const std::vector<std::string> walletNames = { "Child (99)", "Adult (200)", "Giant (500)", "Tycoon (999)" };
...
void DrawDungeonItemButtonForTemplate(HarpoonTemplates::Template* t, uint32_t item, uint32_t scene) {
    auto it = itemMapping.find(item);
    if (it == itemMapping.end())
        return;
    const ItemMapEntry& entry = it->second;
...
void DrawQuestStatusTab(HarpoonTemplates::Template* t) {
    ...
    DrawGroupWithBorder(
        [&]() {
            ...
            if (dungeonItemsScene != SCENE_JABU_JABU_BOSS && dungeonItemsScene < (int32_t)ARRAY_COUNT(t->dungeonKeys)) {
                ...
                if (keyMap != itemMapping.end()) {
                    ImGui::Image(std::dynamic_pointer_cast<Fast::Fast3dGui>(
                                     Ship::Context::GetRawInstance()->GetWindow()->GetGui())
                                     ->GetTextureByName(keyMap->second.name),
...
void DrawHeader() {
    ...
    if (ImGui::BeginCombo("##remotePeer", current.c_str())) {
        if (harpoon != nullptr) {
            for (auto& [cid, c] : harpoon->clients) {
                if (cid == harpoon->ownClientId || !c.online)
                    continue;
    ...
    ImGui::SameLine();
    if (ImGui::Button("Refresh") && sTargetCid != 0)
        RequestPeek(sTargetCid);
    ImGui::SameLine();
    ...
    if (ImGui::Button("Apply ▸ peer") && snap != nullptr && harpoon != nullptr) {
        harpoon->SendJsonToRemote(HarpoonTemplates::BuildTemplateApplyPayload(sTargetCid, *snap));
    ...
    if (sTargetCid != 0) {
        ...
        if (age < 0.0f) {
            ImGui::TextColored(ImVec4(1.0f, 0.6f, 0.2f, 1.0f), "Snapshot: pending… (press Refresh)");
        } else {
            ImGui::TextColored(ImVec4(0.7f, 0.85f, 1.0f, 1.0f), "Snapshot age: %.1fs", age);
        }
    }
    ImGui::Separator();
}
...
} // matches vanilla's no-op pattern between tabs
...
nlohmann::json BuildPeekRequestPayload(uint32_t targetClientId) {
    ...
    d["requesterClientId"] = Harpoon::Instance != nullptr ? Harpoon::Instance->ownClientId : 0u;
    p["data"] = d;
    return p;
}
...
nlohmann::json BuildPeekResponsePayload(uint32_t requesterClientId) {
    ...
    d["ownerClientId"] = Harpoon::Instance != nullptr ? Harpoon::Instance->ownClientId : 0u;
...
void HandlePeekRequest(const nlohmann::json& data) {
    ...
    if (target != Harpoon::Instance->ownClientId)
        return;
    if (reqCid == 0 || reqCid != Harpoon::Instance->hostClientId) {
        SPDLOG_WARN("[Harpoon][RemoteEdit] peek request from non-host cid={} ignored", reqCid);
        return;
    }
...
void HandlePeekResponse(const nlohmann::json& data) {
    ...
    if (requester != Harpoon::Instance->ownClientId)
        return;
    if (owner == 0)
        return;
...
void RemoteSaveEditorWindow::DrawElement() {
    ...
    if (!IsHostLocal()) {
        ImGui::TextColored(ImVec4(1.0f, 0.4f, 0.4f, 1.0f), "The Remote Save Editor is host-only.");
        ImGui::TextWrapped("Only the room host (GM) can peek and apply peer save state. "
    ...
    HarpoonTemplates::Template* snap = GetActiveSnapshot();
    if (snap == nullptr) {
        ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.8f, 1.0f), "Select a peer to load their save snapshot.");
        ImGui::PopFont();
        PopStyleTabs();
        return;
    }

    if (ImGui::BeginTabBar("RemoteSaveContextTabBar", ImGuiTabBarFlags_NoCloseWithMiddleMouseButton)) {
        ResetBaseOptions();
