...
nlohmann::json TemplateToJson(const Template& t) {
    ...
    for (auto v : t.items)
        o["items"].push_back((int)v);
    o["ammo"] = nlohmann::json::array();
    for (auto v : t.ammo)
        o["ammo"].push_back((int)v);
    ...
    for (auto v : t.dungeonItems)
        o["dungeonItems"].push_back((int)v);
    o["dungeonKeys"] = nlohmann::json::array();
    for (auto v : t.dungeonKeys)
        o["dungeonKeys"].push_back((int)v);
    ...
    for (auto v : t.eventChkInf)
        o["eventChkInf"].push_back((int)v);
    o["itemGetInf"] = nlohmann::json::array();
    for (auto v : t.itemGetInf)
        o["itemGetInf"].push_back((int)v);
    o["infTable"] = nlohmann::json::array();
    for (auto v : t.infTable)
        o["infTable"].push_back((int)v);
    o["eventInf"] = nlohmann::json::array();
    for (auto v : t.eventInf)
        o["eventInf"].push_back((int)v);
    o["randomizerInf"] = nlohmann::json::array();
    for (auto v : t.randomizerInf)
        o["randomizerInf"].push_back((int)v);

    // File metadata.
    o["fileNum"] = (int)t.fileNum;
    o["playerName"] = nlohmann::json::array();
    for (auto v : t.playerName)
        o["playerName"].push_back((int)v);
    ...
    for (auto v : t.highScores)
        o["highScores"].push_back((int)v);
    o["gsFlags"] = nlohmann::json::array();
    for (auto v : t.gsFlags)
        o["gsFlags"].push_back((int)v);
...
void CaptureLocalState(Template& t) {
    ...
    t.randomizerInf.assign(gSaveContext.ship.randomizerInf, gSaveContext.ship.randomizerInf + kRandInfLen);
...
void LoadAll() {
    ...
    SPDLOG_INFO("[Harpoon][Templates] loaded {} templates from {}", (int)sTemplates.size(), dir.string());
}
...
void ApplyToSave(const Template& t) {
    for (size_t i = 0; i < (size_t)ARRAY_COUNT(t.items); i++) { // Skijer's NEI
        if (i < 24)
            ...
            Nei_SetOwnedItem((uint8_t)i, t.items[i]);
    }
    for (size_t i = 0; i < (size_t)ARRAY_COUNT(t.ammo); i++) {
...
bool ApplyToPeer(uint32_t targetClientId, const std::string& name) {
    const Template* t = Find(name);
    if (t == nullptr || Harpoon::Instance == nullptr)
        return false;
