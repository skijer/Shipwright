...
static void HarpoonMainMenu(WidgetInfo& info) {
    ...
    if (isReady && !inRoom) {
        ...
        if (ImGui::Button("Create Room")) {
            SPDLOG_INFO("[Harpoon][Menu] Create Room clicked: name='{}' gm='{}' pass={} connected={} hasGamemodes={}",
                        roomNameBuf, gameModeBuf, roomPassBuf[0] ? "(set)" : "(empty)", harpoon->isConnected,
                        hasGamemodes);
        ...
        if (!hasGamemodes) {
            ImGui::SameLine();
            ImGui::TextColored(ImVec4(1.0f, 0.4f, 0.4f, 1.0f), "(button disabled — no gamemodes detected)");
        }
        ...
        if (!harpoon->roomList.empty()) {
            ...
            for (auto& room : harpoon->roomList) {
                ...
                if (!gmInstalled) {
                    ImGui::TextColored(ImVec4(1.0f, 0.5f, 0.5f, 1.0f), "  %s (%s) [%d/%d] %s%s", room.name.c_str(),
                                       room.gameMode.c_str(), room.playerCount, room.maxPlayers, room.state.c_str(),
    ...
    if (inRoom) {
        ...
        bool isHost = (harpoon->ownClientId != 0 && harpoon->ownClientId == harpoon->hostClientId);
        bool isAdmin = isHost;
        ...
        if (harpoon->currentRoomGameMode == "prop_hunt") {
            ...
            if (isAdmin) {
                ...
                if (ImGui::Combo("##MapMode", &settings.mapSelectMode, mapModeNames, IM_ARRAYSIZE(mapModeNames))) {
                    harpoon->mapSelectMode = (HarpoonMapSelectMode)settings.mapSelectMode;
                }

                if (settings.mapSelectMode == 0) {
                    const char* mapNames[] = {
                        "Kakariko Village", "Death Mountain",   "Clock Town",     "Gerudo Fortress", "Forest Temple",
                        "Zora's River",     "Dodongo's Cavern", "Ganon's Castle", "Kokiri Forest",
                    };
                    ImGui::Text("Map:");
                    ImGui::SameLine();
                    ImGui::SetNextItemWidth(180);
                    ImGui::Combo("##HostMap", &settings.selectedMap, mapNames, IM_ARRAYSIZE(mapNames));
                }
            ...
            if (ImGui::Button("Start Game")) {
                ...
                if (candidates.empty())
                    candidates.push_back(harpoon->ownClientId);
                ...
                HarpoonPropHunt::Role myRole = seekerSet.count(harpoon->ownClientId) > 0 ? HarpoonPropHunt::Role::Seeker
                ...
                for (auto& [cid, c] : harpoon->clients) {
                    if (cid == harpoon->ownClientId)
                        continue;
                    HarpoonPropHunt::Role r =
                        seekerSet.count(cid) > 0 ? HarpoonPropHunt::Role::Seeker : HarpoonPropHunt::Role::Hider;
                    harpoon->SendJsonToRemote(HarpoonPropHunt::BuildRoleAssignPayload(cid, r));
                }

                // 3. Schedule the hide phase to start once everyone has loaded.
                harpoon->SendJsonToRemote(HarpoonPropHunt::BuildHidePhaseBeginPayload(settings.hideSeconds * 20));
            ...
            ImGui::Spacing();
            ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f), "Hider controls in-game: D-Left = next category, "
            ...
            for (auto& [cid, c] : harpoon->clients) {
                ...
                if (c.role == "hider") {
                    rolColor = ImVec4(0.5f, 1.0f, 0.5f, 1.0f);
                    rolTag = "HIDER";
                } else if (c.role == "seeker") {
                    rolColor = ImVec4(1.0f, 0.5f, 0.5f, 1.0f);
                    rolTag = "SEEKER";
                }
                ...
                if (!c.pendingRole.empty()) {
                    ImGui::SameLine();
                    ImVec4 pendCol =
                        (c.pendingRole == "seeker") ? ImVec4(1.0f, 0.6f, 0.6f, 1.0f) : ImVec4(0.6f, 1.0f, 0.6f, 1.0f);
                    ImGui::TextColored(pendCol, "→ pending: %s", c.pendingRole == "seeker" ? "SEEKER" : "HIDER");
                }
        ...
        if (harpoon->currentRoomGameMode == "triforce_thief") {
            ...
            if (maps.empty()) {
                ...
                ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f), "  %s", maps[selectedMap].description.c_str());
                ...
                if (isAdmin) {
                    ...
                    ImGui::SliderInt("##TTwinSec", &HarpoonTriforceThief::GetLocalState().roundWinSeconds, 15, 99,
                                     "%d s");
                    ...
                    ImGui::SameLine();
                    ImGui::SetNextItemWidth(180);
                    if (ImGui::Combo("##TTMapMode", &ttMapSelectMode, mapModeNames, IM_ARRAYSIZE(mapModeNames))) {
                    ...
                    for (auto& [cid, c] : harpoon->clients) {
                        ...
                        if (c.team.empty()) {
                            anyTeamless = true;
                            if (!teamlessHint.empty())
                                teamlessHint += ", ";
                            teamlessHint += c.name.empty() ? ("cid" + std::to_string(cid)) : c.name;
                        }
                    }
                    ...
                    ImGui::EndDisabled();
                    if (anyTeamless) {
                        ImGui::TextColored(ImVec4(1.0f, 0.4f, 0.4f, 1.0f), "  No team picked yet: %s",
                                           teamlessHint.c_str());
                    }
                    ImGui::EndDisabled();
                }
                ...
                bool roundInFlightTT =
                    HarpoonTriforceThief::GetLocalState().inRound && !HarpoonTriforceThief::GetLocalState().roundEnded;
                ImGui::BeginDisabled(!isHost || !roundInFlightTT);
                if (ImGui::Button("Finish Round")) {
                    auto payload = HarpoonTriforceThief::BuildRoundResultPayload(
                        harpoon->ownClientId, HarpoonTriforceThief::GetLocalState().roundIndex);
                ...
                ImGui::Separator();
                ImGui::TextColored(ImVec4(0.8f, 0.9f, 1.0f, 1.0f), "Teams (lobby only):");
                const bool inLobby = (harpoon->gameState == HARPOON_STATE_LOBBY);
                ...
                ImGui::EndDisabled();
                if (!inLobby) {
                    ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f), "  Team switching disabled mid-round.");
                }
                ...
                for (auto& [cid, c] : harpoon->clients) {
                    ...
                    ImGui::SameLine();
                    ImGui::TextColored(col, "%s%s", c.name.c_str(), c.self ? " (you)" : "");
                    ImGui::PopID();
                }
            }
        }
        ...
        if (isHost && harpoon->currentRoomGameMode == "rpg") {
            ...
            if (ImGui::CollapsingHeader("Templates")) {
                ...
                if (tpls.empty()) {
                    ImGui::TextColored(ImVec4(0.6f, 0.6f, 0.6f, 1.0f), "  (no templates saved yet)");
                }
            ...
            if (ImGui::CollapsingHeader("Remote Save Editor")) {
                ImGui::TextWrapped("Edit any peer's save state. Pick a player below to "
                ...
                for (auto& [cid, c] : harpoon->clients) {
                    ...
                    std::string label = c.name.empty() ? ("cid" + std::to_string(cid)) : c.name;
                ...
                if (!anyPeer) {
                    ImGui::TextColored(ImVec4(0.6f, 0.6f, 0.6f, 1.0f), "  (no peers connected)");
                }
            }
