...
#undef LOCATIONS_PER_SHOP
        ...
        // BOTH worlds and this stage must stand down — otherwise it claims OoT's 12 song locations
        // here, before the combo hook runs further down, and the shared 24-spot pool loses half its
        // spots (and every song already in a spot gets a second copy from the cross-placement).
        if (FleetCombo_SharedSongsMode() == 0 && ctx->GetOption(RSK_SHUFFLE_SONGS).IsNot(RO_SONG_SHUFFLE_ANYWHERE) &&
